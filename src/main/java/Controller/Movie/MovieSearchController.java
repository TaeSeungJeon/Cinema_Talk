...
public class MovieSearchController implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//검색어 2글자 이상 검사
			...
			movie.setSearchWords(searchWords);
			
			MovieSearchService movieSearchService = new MovieSearchServiceImpl();
			
			int totalCount= movieSearchService.getRowCount(movie, searchOption);	//검색전 총레코드 개수,검색후 레코드 개수
			List<MovieDTO> movies = movieSearchService.getMovieDTOList(movie, searchOption);
