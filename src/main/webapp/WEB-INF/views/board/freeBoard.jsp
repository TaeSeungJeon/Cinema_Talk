		...
		<main>
			<div class="comm-header">
				<div>
					...
					<p
						style="color: var(--text-sub); margin-top: 5px; font-weight: 500;">
						영화 팬들과 자유롭게 소통하세요</p>
				</div>
			...
			<div class="notice-bar">
				...
				<c:choose>
					<c:when test="${not empty latestNotice}">
						<a
							href="${pageContext.request.contextPath}/postDetail.do?boardId=${latestNotice.boardId}&boardType=${latestNotice.boardType}"
							style="color: #64748b; text-decoration: none;"> <c:out
								value="${latestNotice.boardTitle}" />
						</a>
					</c:when>
					<c:otherwise>
					...
					<article class="post-card">
						<div class="post-card-header">
							...
							<c:if test="${not empty board.movieTitle}">
								<span class="movie-tag-container"> <c:choose>
										<%-- 삭제된 영화 --%>
										<c:when
											test="${(empty board.movieId or board.movieId eq 0 ) }">
											<button type="button"
												onclick="location.href='movieDetail.do?movieId=${board.movieId}'"
												class="movie-tag disabled-tag" disabled
												title="정보가 삭제된 영화입니다">#${board.movieTitle}</button>
				...
				<div style="display: flex; gap: 10px;">
					...
					<div style="flex: 2; position: relative; width: 100%;"
						...
						<input type="hidden" name="movieId" class="movie-id-hidden"
							value="${movieId}"> <input class="movie-search"
